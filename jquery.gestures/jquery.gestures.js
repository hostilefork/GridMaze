(function($)
{
	...
	// NOTE: THIS FILE CONTAINS CUSTOM PATCHES FOR GRIDMAZE PROJECT
	// SEE "+GM" and "-GM" REMARKS
	...
	$.fn.gestures = function (settings) {
				...
				if (xLength > config.tolerance || yLength > config.tolerance) {
					...
					if (direction != "falsealarm") {
						...
						if (directionArray.length == 0) {
							...
							/* config.eventHandler(direction); */ // -GM
							config.eventHandler.call(eventStartElement, direction); // +GM
						} else{
		...
		// Starts tracking
		/* function StartTracking() { */ // -GM
		function StartTracking(element) { // +GM
			if (!underEvent) {
				underEvent = true;
				eventStartElement = element; // +GM
		...
		function EndTracking() {
			...
			if (underEvent) {
				...
				if (config.advancedShapes) {
					...
					// Simple Event
					/* config.eventHandler(GetEvent(eventStartX, eventStartY, eventEndX, eventEndY)); */ // -GM
					config.eventHandler.call(eventStartElement, GetEvent(eventStartX, eventStartY, eventEndX, eventEndY)); // +GM
				}
				// Track Mouse Trail
				if (config.showTrail) {
					ClearTrail();
